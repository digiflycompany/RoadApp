...
class RecoveryRepo {
  ...
  Future<ApiResult<SendCodeResponse>> verifyEmail(
  ...
  Future<ApiResult<SendCodeResetResponse>> verifyEmailToReset(
  ...
  Future<ApiResult<SendEmailResponse>> resetPassword(
      String token, ResetPasswordRequestBody body) async {

    try {
      final response = await _apiService.resetPassword(token, body);
      return ApiResult.success(response);
    } catch (error) {
      DefaultLogger.logger.e(error);
      return ApiResult.failure(ErrorHandler.handle(error));
